...
export default function VersionHistory(props) {
  ...
  const path = decodeURIComponent(
    useRecoilValue(searchParamAtomFamily("path")),
  );
  ...
  const setReleaseNamed = useRecoilCallback(
    ({ set, snapshot }) =>
      async ({ doenetId, versionId, driveId, folderId, itemId }) => {
        const { data } = await axios.get("/api/releaseVersion.php", {
          params: { doenetId, versionId },
        });
        ...
        set(folderDictionary({ driveId, folderId }), (was) => {
          ...
          newFolderInfo.contentsDictionaryByDoenetId[doenetId].isReleased =
            isReleased;
  ...
  const setAsCurrent = useRecoilCallback(
    ({ snapshot, set }) =>
      async ({ doenetId, versionId }) => {
  ...
  const saveVersion = useRecoilCallback(
    ({ snapshot, set }) =>
      async (doenetId) => {
        ...
        let newDBVersion = { ...newVersion, doenetML, doenetId };
        ...
        axios
          .post("/api/saveNewVersion.php", newDBVersion)
          .then((resp) => {
  ...
  const saveAndReleaseCurrent = useRecoilCallback(
    ({ snapshot, set }) =>
  ...
  const setSelectedVersionId = useRecoilCallback(
    ({ snapshot, set }) =>
      async ({ doenetId, versionId, isCurrentDraft }) => {
        //Update menus
    ...
    <>
      ...
      <div>
        <Button
          disabled={currentDraftSelected}
          onClick={() =>
            setAsCurrent({ doenetId, versionId: version.versionId })
          }
